...
class Siamese(PreTrainedModel):
    def __init__(self,config,code_bert='microsoft/codebert-base'):
        ...
        # Linear layer for concatenation processing
...
def train_single(df,df_test, data_name,model_name,encoder_path,encoder_name,args,split=[0.8,0.1,0.1],enhance_type="none",enhance_llm="none",data_enhance=None,criterion = nn.CrossEntropyLoss()):
    ...
    data_enhance : Data enhancement dataset
...
result = pd.DataFrame(result).to_excel("/gemini/output/1-20_jina_result_record.xlsx",index=False)
        # # No sample - code
