...
class Args:
    def __init__(self,seed,enhance_llm) -> None:
        ...
        # Model name_encoder_dataset_enhanced_dataset_model_used_for_data_augmentation
...
def train_single(df,df_test, data_name,model_name,encoder_path,encoder_name,args,split=[0.8,0.1,0.1],enhance_type="none",enhance_llm="none",data_enhance=None,criterion = nn.CrossEntropyLoss()):
    """
    ...
    enhance_llm : Model name used for data enhancement*
    ...
    if enhance_type!="none" and enhance_llm=="none":
        print("Using data augmentation but the model for data augmentation is empty, please re-enter!")
...
for s in range(2014,2019):
    ...
    for dn in ["EBT","RETRO","eTOUR","iTrust"]:
        ...
        df_test = pd.read_csv(f'/gemini/data-3/1—20_full_{dn}_link.csv')
        data_enhance=pd.read_excel(f"{args.enhance_data_path}/full_pure_gen_requirement_{dn}.xlsx")
        model_name = "Single"
        ...
        result["enhance_type"].extend(['none']*len(df_test))
        result["enhance_llm"].extend(['none']*len(df_test))
        ...
        # result["code"].extend(df_test['target_text'].tolist())
        # result["label2"].extend(df_test['label'].tolist())
        # result["label"].extend(record["label"])
        # result["socre"].extend(record["score"])
result = pd.DataFrame(result).to_excel("/gemini/output/1-20_codebert_result_record.xlsx",index=False)
        # # No sample - code
