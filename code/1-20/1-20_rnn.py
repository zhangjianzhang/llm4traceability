...
# Define output file path (can include subfolders)
output_file = "rnn_result_record.xlsx"

# Ensure target directory exists
output_dir = os.path.dirname(output_file)
if output_dir:  # Prevent output_file from being a current directory file (no directory part)
...
class RNNTracer(nn.Module):
    def __init__(self,model_name,input_size,args,hidden_size=30,dropout=0,):
        ...
        self.dp = dropout
        self.args = args
        # Process concatenated linear layer
        if "bi" in model_name:
...
#         self.accumulation_steps = 2
        # Model name_encoder_data_set_enhancement_data_set_model_used_for_data_augmentation
        # Training function parameters should include model name_encoder_dataset_name_dataset_enhancement_method_model_used_for_data_augmentation
args = Args(2024)
# set_seed(args.seed)
# args.device
...
def train_rnn(df,data_name,model_name,args,split=[0.8,0.1,0.1],enhance_type="none",enhance_llm="none",data_enhance=None,test_data=None,criterion = nn.CrossEntropyLoss()):
    """
    ...
    enhance_llm : Model name used for data enhancement* (required)
...
#     model = Single(BertConfig(),code_bert=encoder_path)
    ...
    #=============================================================================================
    result_record = pd.read_excel("rnn_result_record.xlsx")
     #=============================================================================================
...
#     tokenizer=AutoTokenizer.from_pretrained(encoder_path)
    ...
    test_data_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False,pin_memory=False)
...
#     result["encoder_name"] = encoder_name
    ...
    result_record.to_excel("/home/bigchill/trace_rag/rnn_result_record.xlsx",index=False)
     #=============================================================================================
    return record
...
for seed in range(2014,2019):
    ...
    llm = '-' #["claude3","gemini","gpt3.5","gpt4o"]:
    # Training
    for dn in ["RETRO","eTOUR","iTrust","EBT"]:
        t = 'none'#["req","code","req(example)","code(example)"]:
        for model in ["lstm"]:#['lstm','gru','bi_lstm','bi_gru']:
            print(f"-----{dn}--{model}--{seed}-----------")
            # df = pd.read_csv(f"myTraceBERT/datasets/raw/full_{dn}_link.csv")
            df= pd.read_csv(f'/home/bigchill/trace_rag/datasets/raw/full_{dn}_link.csv') 

            # enhance_df = pd.read_excel(f"myTraceBERT/datasets/{llm}/{type2df[t]}{dn}.xlsx")
            # enhance_df = pd.concat([pd.read_excel(f"/home/bigchill/trace_rag/datasets/{llm}/{type2df[t]}{dn}.xlsx") for dn_ in ["EBT","RETRO","eTOUR","iTrust"] if dn_ != dn])
            enhance_df = None
            test_data = pd.read_csv(f'/home/bigchill/trace_rag/datasets/raw/1—20_full_{dn}_link.csv')

            record = train_rnn(df=df,data_name=dn,model_name=model,args=args,enhance_type=t,enhance_llm=llm,data_enhance=enhance_df,test_data=test_data)
            result["model_name"].extend([model]*len(test_data))
            result["data_name"].extend([dn]*len(test_data))
            result["enhance_type"].extend([t]*len(test_data))
            result["enhance_llm"].extend([llm]*len(test_data))
            result["seed"].extend([seed]*len(test_data))
            result["req"].extend(test_data['source_text'].tolist())
            result["code"].extend(test_data['target_text'].tolist())
            result["label"].extend(test_data['label'].tolist())
            result["pred"].extend(record["pred"])
            result["socre"].extend(record["score"])
result = pd.DataFrame(result).to_excel("1-20_rnn_result.xlsx",index=False)