...
if output_dir:  # Prevent output_file from being a file in current directory (no directory part)
...
def train_rnn(df,data_name,model_name,args,split=[0.8,0.1,0.1],enhance_type="none",enhance_llm="none",data_enhance=None,test_data=None,criterion = nn.CrossEntropyLoss()):
    ...
    split : Proportion of data set division
        ...
        for dn in ["RETRO","eTOUR","iTrust","EBT"]:
            for t in ["req"]:#["req","code","req(example)","code(example)"]:
                for model in ["lstm"]:#['lstm','gru','bi_lstm','bi_gru']:
                    ...
                    df= pd.concat([pd.read_csv(f'/home/bigchill/trace_rag/datasets/raw/full_{dn_}_link.csv') for dn_ in ["EBT","RETRO","eTOUR","iTrust"] if dn_ != dn])
                    ...
                    test_data = pd.read_csv(f'/home/bigchill/trace_rag/datasets/raw/no_balance_full_{dn}_link.csv')
...
result = pd.DataFrame(result).to_excel("lopo_rnn_result.xlsx",index=False)