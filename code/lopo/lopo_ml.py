        ...
        for d in ["EBT","eTOUR","iTrust","RETRO"]:
            for t in ["req"]: #["req","code","req(example)","code(example)"]:
                for model_name in ["LR","SVM","KNN"]: 
                    ...
                    pred_result = ml.train(df,enhance_type=t,enhance_llm=llm,data_enhance=enhance_df,test_data=test_data)
...
pd.DataFrame(result).to_excel("lopo_ml_enhance_result_record.xlsx",index=False)